"""
DECODER PRE-TRAINING SCRIPT — train FrameDecoder on real replay data.
...
What it does:
    1. Loads the frozen HRVQ tokenizer (codebook embeddings are fixed).
    2. Loads raw frames + pre-computed token indices from the replay buffer.
    3. Trains FrameDecoder with weighted MSE at 3 reconstruction levels.
    4. Saves checkpoint to checkpoints/decoder_best.pt
    5. Saves a visual reconstruction grid to checkpoints/decoder_recon_grid.png

Why pre-train separately:
    - Stable supervised phase (no RL noise in the loss).
    - Decoder gets frozen during policy training → policy cannot "cheat" by
      having the decoder encode reward-relevant patterns directly.
    - Training time: ~15-30 min on GPU, much less on CPU.

SUCCESS CRITERION:
    Open decoder_recon_grid.png.  The 'full' reconstruction column should clearly
    show a paddle (bright vertical bar) and a ball (bright dot).  If it looks like
    a uniform grey blob, the CNN encoder has destroyed spatial information at
    tokenization time and pixel decoding cannot help — report this to the team.
"""
...
def weighted_recon_loss(
    pred_coarse : torch.Tensor,   # (B, 1, 84, 84)
    pred_mid    : torch.Tensor,
    pred_full   : torch.Tensor,
    target      : torch.Tensor,   # (B, 1, 84, 84)
    w_coarse    : float = 0.2,
    w_mid       : float = 0.3,
    w_full      : float = 0.5,
) -> torch.Tensor:
    ...
    Full level has highest weight because it uses all residual information and
    is what the policy will primarily use.  Coarse level is a cheap auxiliary
    that prevents the decoder from ignoring L0 alone.
    """
...
def train_decoder(args):
    ...
    print()
    print("  *** IMPORTANT: Open decoder_recon_grid.png and verify that ***")
    print("  *** the 'full' column shows a visible paddle and ball.      ***")
    print("  *** If it's a grey blob, spatial info was lost at encoding. ***")

    return best_val_loss


def _save_reconstruction_grid(decoder, hrvq, dataset, device, save_dir, n=8):
    """Save a grid: columns = [original, coarse, mid, full] for n random frames."""
    try:
        import matplotlib
        matplotlib.use('Agg')
