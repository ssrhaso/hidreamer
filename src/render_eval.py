...
def load_all(checkpoint_path, config_path="configs/policy.yaml", device="cuda"):
    """ LOAD FROZEN MODELS AND TRAINED POLICY FROM CHECKPOINT """
...
def run_episode(wm, hrvq, encoder, feat_ext, actor, env, device, greedy=True):
    """ RUN ONE EPISODE AND RETURN FRAMES, ACTIONS, AND REWARDS """
    obs, _ = env.reset()
    ...
    while not done:
        # USE LAST CHANNEL OF FRAME STACK FOR DISPLAY
